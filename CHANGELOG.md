...
## 2026-05-22 — [#65](https://github.com/testerjp/miniGestures/pull/65) Apply settings changes to already-open tabs
...
## 2026-05-19 — [#60](https://github.com/testerjp/miniGestures/pull/60) Remove the gesture-field tooltip
- Each gesture field on the options page showed a custom tooltip (`.gestureHint`, added in #59) that repeated the available-directions note whenever the field held an invalid character and had focus. The same note already appears as a permanent `<h2>` heading at the top of the page, so the per-field tooltip was redundant.
- The `.gestureHint` element and its CSS are removed, along with the field's `aria-describedby` link to it. Invalid input is still flagged inline by the red `:invalid` `box-shadow`, and the `[UDLRudlr]*` `pattern` is unchanged, so `save_options()` still skips a field holding an invalid character.
...
- `save_options()` no longer persists a gesture field that holds an invalid character: like an invalid color code it skips only that field's write, keeping the previously stored mapping, while the other settings still save.
- Restored the per-row auto-save wiring: `f38cc5b` added `scheduleSave` to each gesture input, but the line was dropped in the #57 merge, so mapping changes had stopped persisting. The inputs are wired to `scheduleSave` again, matching what the `wireAutoSave()` comment already documents.
...
- The `th.trailSub` rule was re-scoped to `#optsTab th.trailSub` so it keeps higher specificity than the new `#optsTab th` rule. Its `padding-left: 2.5em` indentation was dropped, so the "Show Gesture Trails" subsection labels align with the other settings labels; the subsection is still distinguished by its non-bold `font-weight`.
...
- Dropped the trailing colon from all five settings-table labels (`gestureButtonLabel`, `showTrails`, `gestureColor`, `gestureWidth`, `gestureOpacity`) in `options.html` and in every `_locales/<locale>/messages.json`, including the full-width `：` used by `zh_CN`/`zh_TW` and the spaced ` :` used by `fr`.
...
- Gesture-table rows now store their command code in a `data-cmd` attribute instead of being matched by the (previously English-only) label text, so saving still works once the labels are localized. The color and trigger-button `<option>` `value`s are unchanged — only their visible text is translated — so saved settings are unaffected.
- The PayPal donation form is deliberately left in English: the amount tiers (`Small/Medium/Large Donation $1/$5/$10 USD`) and the `Donation Levels` label belong to a payment widget tied to PayPal's encrypted button, so they are not run through the translation layer. Only the plain "If you like this, feel free to donate!" heading above the form is localized.
...
- Removed the now-obsolete Chrome `background.scripts` warning note from README "Install on Chrome".
...
## 2026-05-15 — [#33](https://github.com/testerjp/miniGestures/pull/33) Simplify README "Package for Firefox" section
